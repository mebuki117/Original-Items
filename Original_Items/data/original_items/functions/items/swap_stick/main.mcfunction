...
execute as @a[scores={origitem_click=1}] if data entity @s {SelectedItem:{tag:{originalitems:2}}} at @s run tp @p[gamemode=survival,distance=..30] @e[type=minecraft:armor_stand,tag=swap_dummy,sort=nearest,limit=1]
